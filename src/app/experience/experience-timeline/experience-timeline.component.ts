...
@Component({
  ...
  styleUrls: [
    ...
    "experience-timeline.component.reponsivity.scss",
  ],
})
export class ExperienceTimelineComponent implements OnInit, OnDestroy {
  ...
  updateTimelineNavigation() {
    const activePreviousElem = this.line.nativeElement.querySelector(
      ".milestone.active.current"
    );
    this.renderer.removeClass(activePreviousElem, "current");

    const targetElem = this.line.nativeElement.querySelector(
      `div[id-position="${this.currentPosition}"]`
    );
  ...
  populateExperienceTimeline(): void {
    ...
    if (dates && dates.length < 2) {
      ...
      const daysBetween: number = this.daysBetween(
        dates[0],
      ...
      this.renderer.appendChild(
        ...
        this.createMilestone(1, 0, dates[0])
      );
      ...
      for (i = 1; i < lastFrameLoop; i++) {
        ...
        const milestoneElement = this.createMilestone(
          i + 1,
          periodWidth,
          dates[i]
        );
  ...
  createCurrentTriangle(_index: number): any {
    const milestoneElement = this.renderer.createElement("div");
    this.renderer.addClass(milestoneElement, "milestone");
    this.renderer.addClass(milestoneElement, "active");
  ...
  createLabelElement(date: string): string {
    ...
    const safariDateFormatterPipeValue =
      safariDateFormatterPipe.transform(date);

    const localizedDatePipe = new LocalizedDatePipe(this.locale);

    let month: any = localizedDatePipe.transform(
      safariDateFormatterPipeValue,
      "MMM"
    );
    ...
    const year = localizedDatePipe.transform(
      safariDateFormatterPipeValue,
      "yyyy"
    );
